...
// import { filterDataLogged } from "../components/FilterComponent";
// let reversedata =filterDataLogged
...
function DashboardScreen({ filters ,JsonData }) {
  ...
  const DashboardData = regData.map((element) => {
    ...
    // const countryList = filters.countries.length
    //   ? [...country].filter((item) => filters.countries.includes(item))
    //   : [...country];
